...
iat1 = load_iat("/home/labtlc/openfaas_k3s_rl/iat_4_A.txt")
...
function response()
  ...
  counter = counter+1
   ...
   local filelat = assert(io.open("latreport4.csv", "w"))
   filelat:write('Perc')
   filelat:write('\n')
   filelat:write(latency:percentile(99.0))
   filelat:close()
   local file = assert(io.open("report4.csv", "w"))
   ...
   local summaryfile = assert(io.open("summary4.csv","w"))
