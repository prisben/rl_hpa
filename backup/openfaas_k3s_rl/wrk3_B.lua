...
iat1 = load_iat("/home/labtlc/openfaas_k3s_rl/iat_3_A.txt")
...
function response()
  ...
  counter = counter+1
   ...
   local filelat = assert(io.open("latreport3.csv", "w"))
   filelat:write('Perc')
   filelat:write('\n')
   filelat:write(latency:percentile(99.0))
   filelat:close()
   local file = assert(io.open("report3.csv", "w"))
   ...
   local summaryfile = assert(io.open("summary3.csv","w"))
